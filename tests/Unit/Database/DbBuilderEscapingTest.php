...
/**
 * SEC-12/13/15 parity for the STATIC DB builder (sibling of QueryBuilder): its
...
{
    private function staticProp(string $name): mixed
    {
    ...
    {
        (new DB())->orderBy('name; DROP TABLE users', 'evil');

        $order = ((array) $this->staticProp('bind_or_filter'))['ORDER BY'];
    ...
    {
        (new DB())->limit('10; DROP')->offset('5 UNION SELECT');

        $bof = (array) $this->staticProp('bind_or_filter');
