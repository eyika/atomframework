...
- **ORM/DB (in progress — contained bugs first, architecture deferred by decision):**
  ...
  - **Still TODO (contained):** BUG-30 (dead JOINs — thread joins through `fetch`), BUG-28 (bindParam dead vs execute($params)), BUG-29 (`_save` `$model[0]` shape), BUG-34 (swallowed PDOExceptions), BUG-36 (`firstOrNew` ignores args), BUG-27 (softdelete double-append), BUG-31 (static DB rewrite — big).
