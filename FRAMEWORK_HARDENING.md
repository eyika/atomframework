...
- **ORM/DB (in progress — contained bugs first, architecture deferred by decision):**
  - **Done:** BUG-24 (`group_concact`→`group_concat` + added `bit_xor`); BUG-25 (`findByEmail`/`findByUsername` were whitelisted with a leading `_` → unreachable via `__callStatic`; fixed); BUG-26 (`restore()` passed a positional `['deleted_at', null]` → wrote columns `0`/`1` and never cleared `deleted_at`; now assoc — fixed in **both** QueryBuilder and DB.php). Tests: `OrmContainedBugsTest`.
  ...
  - **Still TODO (contained):** BUG-28 (bindParam dead vs execute($params)), BUG-29 (`_save` `$model[0]` shape), BUG-30 (dead JOINs — thread joins through `fetch`), BUG-32 (`strpos($name,'_')` at pos 0), BUG-33 (`remove()` returns false on 0 rows), BUG-34 (swallowed PDOExceptions), BUG-35 (`delete(null)`/bare-delete footgun), BUG-36 (`firstOrNew` ignores args). Plus BUG-27 (softdelete double-append), BUG-31 (static DB rewrite — big).
