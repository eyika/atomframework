...
- **Still to clear (2):**
  - **BUG-31 (static `DB` builder rewrite) — LARGE:** the static `DB` builder holds query state in STATIC props (shared across concurrent builders → static `$table` clobber, `$joins`/`$for_update` leaks), has a broken `paginate` (`static::$offset` undefined), and dead `findOr`/`firstWhere`/`firstOrCreate`/`firstOrNew` stubs (BUG-49). Scope: ~89 `static::$` query-state refs across 743 lines → convert to instance state + fix paginate + implement stubs. Works today for single-query usage (smoke test green); bugs surface on interleaved builders. **Recommended as a focused standalone session** (89 mechanical edits — high error-risk at the tail of a long session). App uses `DB::` (console commands etc.) — rewrite is app-affecting.
...
- **Suite: 120 tests / 226 assertions green.**
