...
- **ORM/DB (in progress — contained bugs first, architecture deferred by decision):**
  ...
  - **Still TODO (contained):** BUG-27 (softdelete double-append `deleted_at IS NULL` in `__aggregate`+`_all` → dangling AND in `_paginate`), BUG-31 (static DB rewrite — big). Plus the OBSERVED save-persistence item above.
