...
class Job_Queue
{
	...
	public function addJob(string $payload, int $delay = 0, int $priority = 1024, int $time_to_retry = 60) {
		...
		switch($this->queue_type) {
			...
			case self::QUEUE_TYPE_SQLITE:
				...
				$statement = $this->connection->prepare("INSERT INTO {$table_name} (pipeline, payload, `delay`, added_dt, send_dt, priority, is_reserved, reserved_dt, is_buried, attempts, time_to_retry_dt) VALUES (?, {$field_value}, ?, ?, ?, ?, 0, NULL, 0, 0, ?)");
				$statement->execute([
					$this->pipeline,
					$payload,
                    $delay,
		...
		switch($this->queue_type) {
			...
			case self::QUEUE_TYPE_SQLITE:
                if ($this->connection instanceof PDO || $this->connection instanceof SQLite3) {
                    ...
                    $reserved_dt = gmdate('Y-m-d H:i:s', strtotime('now -1 minutes UTC'));
                    $statement = $this->connection->prepare("SELECT id, {$field}, `delay`, added_dt, send_dt, priority, is_reserved, reserved_dt, is_buried, buried_dt, attempts
                        FROM {$table_name} 
                    ...
                    if(count($result)) {
                        ...
                        $statement = $this->connection->prepare("UPDATE {$table_name} SET is_reserved = 1, reserved_dt = ?, time_to_retry_dt = ?, attempts = attempts + 1 WHERE id = ?");
                        $statement->execute([ $reserved_dt, $retry_time, $job['id'] ]);
                    }
                }
			break;
			...
			case self::QUEUE_TYPE_SQLITE:
                if ($this->connection instanceof PDO || $this->connection instanceof SQLite3) {
                    ...
                    if(count($result)) {
                        ...
                        $delay = intval($result['delay']);
                        $reserved_dt = gmdate('Y-m-d H:i:s', strtotime("now +$delay seconds UTC"));
                        $statement = $this->connection->prepare("UPDATE {$table_name} SET attempts = attempts + 1, time_to_retry_dt = ? WHERE id = ?");
	...
	public function kickJob($job): void {
		...
		switch($this->queue_type) {
			case self::QUEUE_TYPE_MYSQL:
			case self::QUEUE_TYPE_SQLITE:
				$table_name[0] = $this->getSqlTableName();
	...
	*	@param bool $split
	...
	protected function getSqlTableName(): array {
        ...
        if($this->isMysqlQueueType() && isset($this->options['mysql']['failed_table_name'])) {
			$failed_table_name = $this->options['mysql']['failed_table_name'];
		} else if($this->isSqliteQueueType() && isset($this->options['sqlite']['failed_table_name'])) {
			$failed_table_name = $this->options['sqlite']['failed_table_name'];
		}
